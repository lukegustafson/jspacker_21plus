const to_pack = process.argv[2] ?? 'for(A=10;A<100;++A)console.log(123+456%A%21)';
	...
	*/
	...
	function test_regex(regex)
	{
		for(const c of to_pack)
		{
			...
			if(c != escaped && regex.test(escaped))
				return false;
		}
		return true;
	}
	...
	if(!ret.length)
		console.error("Can't find a RegExp compatible with Unicode characters in the source. Try escaping the Unicode characters.");
...
function do_pack(to_pack, regex)
{
	...
	*/
	...
	for(let i = 0; i < 1024; ++i)
	{
		...
		if(to_pack.includes(unpack1))
			high_surrogate_unpack.set(i, unpack1);
		
		const unpack2 = unescape(("%u" + (0xDC00 + i).toString(16).toUpperCase()).replace(regex,""));
		if(to_pack.includes(unpack2))
			low_surrogate_unpack.set(i, unpack2);
	}
...
function display_pack(pack)
{
	...
	{
		let decoded = unescape(escape(c).replace(pack.regex,``)).replace(/[\x00-\x20]/ug, " ").replace(/[^\x20-\x7E]/ug, "?");
		source += decoded;
		if(decoded.length)
			division += "|" + "-".repeat(decoded.length - 1);
	}
...
function main(to_pack, verbose)
{
	...
	for(const regex_to_test of regexes)
	{
		...
		if(verbose)
			display_pack(pack_results);
		if(pack_results.length < best_pack.length)
			best_pack = pack_results;
	}

	if(best_pack.length < Infinity)
	{
		if(verbose)
			console.log("Best result:", best_pack.length);
